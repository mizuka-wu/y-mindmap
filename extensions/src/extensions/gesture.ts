...
export const Gesture = createExtension<GestureOptions>({
  ...
  setup(ctx, options) {
    ...
    const recognizer = new GestureRecognizer((event) => {
      ...
      switch (event.type) {
        case 'pinch':
          ...
          break
        case 'pan':
          if (event.deltaX !== undefined && event.deltaY !== undefined && options.enablePan !== false) {
    ...
    const onDown = (e: PointerEvent) => recognizer.handlePointerDown(e.pointerId, e.clientX, e.clientY)
    const onMove = (e: PointerEvent) => recognizer.handlePointerMove(e.pointerId, e.clientX, e.clientY)
    const onUp = (e: PointerEvent) => recognizer.handlePointerUp(e.pointerId, e.clientX, e.clientY)
    const onCancel = (e: PointerEvent) => recognizer.handlePointerCancel(e.pointerId)
