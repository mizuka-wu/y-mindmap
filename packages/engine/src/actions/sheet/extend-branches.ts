...
export class ExtendBranchesAction extends BaseAction {
  constructor(context) {
    super(context);
    this.actionName = ACTION_NAMES.EXTEND_BRANCHES;
  }
  /**
   * @description 展开 branch
   * 展开指定 branch 时，不需要参数
   * relativeLayer: 展开指定 branch 下的相对 layer 的 branch
   * spread: 递归展开指定 branch 下的所有 branch
   * relativeLayer && spread: 递归展开指定 branch 下的相对 layer 的 branch 下的所有 branch
   * 展开所有： 选中或传入 centralBranch，传入参数 spread = true
   */
  doExecute(
  ...
  queryStatus(
    ...
    if (relativeLayer > 0 || spread) {
      ...
      targets = targets.filter(
        (target) =>
          target.type === VIEW_TYPE.BRANCH && target.model.isCollapse(),
      );
    }
    if (targets.length > 0) {
