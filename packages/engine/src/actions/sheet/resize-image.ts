...
export class ResizeImageAction extends BaseAction {
  constructor(context) {
    super(context);
    this.actionName = ACTION_NAMES.RESIZE_IMAGE;
  }
  /**
   * @param {Array} targets
   * @param {Size} newSize
   **/
  doExecute({ targets = [], newSize }: any = {}) {
    ...
    targets.forEach((target) => {
      ...
      imageModel.resize({
        width: Math.max(0, newSize.width),
        height: Math.max(0, newSize.height),
      });
    });
  }
