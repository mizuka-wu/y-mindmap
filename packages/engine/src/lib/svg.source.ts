/**
 * 这个是SnowBrush导出的svg.ts文件，是魔改的一个 svg.js 引擎，需要确认魔改了哪些地方
 * @todo 逐步替换成 @svgdotjs/svg.js
 */
import { SB_DOM_ID_PREFIX } from '../common/constants/index';
// @ts-nocheck
/* eslint-disable */
...
('use strict');
export const SVG = function (element) {
  if (SVG.supported) {
    element = new SVG.Doc(element);
    if (!Reflect.has(SVG, 'parser')) {
      SVG.prepare(element);
    }
    return element;
  }
...
SVG.eid = function (name) {
  return SB_DOM_ID_PREFIX + name.charAt(0).toUpperCase() + name.slice(1) + SVG.did++;
};
...
SVG.extend = function (initializer?: any, extend?: any) {
  var modules;
  var methods;
  var key;
  var i;
...
SVG.prepare = function (element) {
  ...
  var draw = (body ? new SVG.Doc(body) : element.nested()).size(2, 0);
...
SVG.regex = {
  ...
  /* parse reference id */ reference: /#([a-z0-9\-_]+)/i,
  ...
  /* test for namespaced event */ isEvent: /^[\w]+:[\w]+$/,
};
SVG.defaults = {
  // Default matrix
  matrix: '1 0 0 1 0 0',
    ...
    for (; i < il; ++i) {
      ...
      if (s == 'M' || s == 'L' || s == 'H' || s == 'V' || s == 'C' || s == 'S' || s == 'Q' || s == 'T' || s == 'A') {
        ...
        if (s == 'm') {
          ...
          // @ts-ignore
          segs.replaceItem(
            SVG.parser.path.createSVGPathSegArcAbs(x, y, seg.r1, seg.r2, seg.angle, seg.largeArcFlag, seg.sweepFlag),
            i
          );
        } else if (s == 'z' || s == 'Z') {
          x = x0;
...
SVG.extend(SVG.Number, {
  ...
  toString: function () {
    return (
      (this.unit == '%' ? ~~(this.value * 100000000) / 1000000 : this.unit == 's' ? this.value / 1000 : this.value) +
      this.unit
    );
  },
  valueOf: function () {
...
SVG.ViewBox = function (element) {
  ...
  var width;
  var height;
  var wm = 1; /* width multiplier */
  var hm = 1; /* height multiplier */
  var box = element.bbox();
  var view = (element.attr('viewBox') || '').match(/-?[\d\.]+/g);
  var we = element;
  var he = element;
  ...
  extend: {
    ...
    size: function (width, height) {
      ...
      if (width == null || height == null) {
        p = proportionalSize(this.bbox(), width, height);
      }
    ...
    attr: function (a, v, n) {
      if (a == null) {
        ...
        } else if (SVG.regex.isNumber.test(v.nodeValue)) {
...
SVG.FX = SVG.invent({
      ...
      fx.to = function (pos) {
        ...
        if (akeys == null) {
          ...
          if (element.morphArray && (fx._plot || akeys.indexOf('points') > -1)) {
            /* get destination */
            var box;
            var p = new element.morphArray(fx._plot || fx.attrs.points || element.array);
            /* add size */
            if (fx._size) {
        ...
        this.timeout = svgSetTimeout(function () {
          ...
          fx.render = function () {
            if (fx.situation.play === true) {
              ...
              var pos = time > fx.situation.finish ? 1 : (time - fx.situation.start) / d;
              /* process values */
              fx.to(pos);
  ...
  construct: {
    ...
    animate: function (d, ease, delay) {
      return (this.fx ||= new SVG.FX(this)).stop().animate(d, ease, delay);
    },
...
SVG.on = function (node, event, listener, binding) {
  ...
  var index = (SVG.handlerMap.indexOf(node) + 1 || SVG.handlerMap.push(node)) - 1;
...
SVG.Ellipse = SVG.invent({
  ...
  extend: {
    ...
    x: function (x) {
      if (x == null) {
        return this.cx() - this.attr('rx');
      } else {
        return this.cx(x + this.attr('rx'));
      }
    },
    // Move over y-axis
    y: function (y) {
      if (y == null) {
        return this.cy() - this.attr('ry');
      } else {
        return this.cy(y + this.attr('ry'));
      }
    },
    // Move by center over x-axis
    cx: function (x) {
    ...
    width: function (width) {
      if (width == null) {
        return this.attr('rx') * 2;
      } else {
        return this.attr('rx', new SVG.Number(width).divide(2));
      }
    },
    // Set height of element
    height: function (height) {
      if (height == null) {
        return this.attr('ry') * 2;
      } else {
        return this.attr('ry', new SVG.Number(height).divide(2));
      }
    },
  ...
  extend: {
    ...
    width: function (width) {
      ...
      if (width == null) {
        ...
        return this.attr(this.attr('x1') < this.attr('x2') ? 'x2' : 'x1', b.x + width);
      }
    },
    // Set height of element
    height: function (height) {
      ...
      if (height == null) {
        ...
        return this.attr(this.attr('y1') < this.attr('y2') ? 'y2' : 'y1', b.y + height);
      }
    },
...
SVG.extend(SVG.Polyline, SVG.Polygon, {
  ...
  x: function (x) {
    if (x == null) {
      ...
      return this.move(x, this.bbox().y);
    }
  },
  ...
  y: function (y) {
    if (y == null) {
      ...
      return this.move(this.bbox().x, y);
    }
  },
  // Set width of element
  width: function (width) {
    ...
    if (width == null) {
      ...
      return this.size(width, b.height);
    }
  },
  // Set height of element
  height: function (height) {
    ...
    if (height == null) {
      ...
      return this.size(b.width, height);
    }
  },
  // Set element size to given width and height
  size: function (width, height) {
  ...
  extend: {
    ...
    width: function (width) {
      if (width == null) {
        ...
        return this.size(width, this.bbox().height);
      }
    },
    // Set height of element
    height: function (height) {
      if (height == null) {
        ...
        return this.size(this.bbox().width, height);
      }
    },
  },
  // Add parent method
...
SVG.Text = SVG.invent({
  ...
  create: function () {
    ...
    this.lines = new SVG.Set(); // 创建时初始化lines, 否则某些情况下缺少lines导致rebuild报错
    /* set default font */
...
SVG.FeDropShadow = SVG.invent({
  ...
  extend: {
    ...
    stdDeviation: function (num) {
      return this.attr('stdDeviation', num);
    },
    // Relative move over x and y axes
    dmove: function (x, y) {
      return this.dx(x).dy(y);
    ...
    floodColor: function (color) {
      return this.attr('flood-color', color);
    },
    // The flood-opacity attribute indicates the opacity value to use across the current filter primitive subregion.
    floodOpacity: function (opacity) {
      return this.attr('flood-opacity', opacity);
    },
  },
});
SVG.extend(SVG.Defs, {
...
SVG.extend(SVG.Parent, SVG.Text, SVG.FX, {
  ...
  font: function (o) {
    for (var k in o) {
      ...
      } else if (k == 'size' || k == 'family' || k == 'weight' || k == 'stretch' || k == 'variant' || k == 'style') {
...
SVG.Set.inherit = function () {
  ...
  methods.forEach(function (method) {
    SVG.Set.prototype[method] = function () {
      ...
      if (method == 'animate') {
        return (this.fx ||= new SVG.SetFX(this));
      } else {
        return this;
      }
    };
...
function at(o, pos) {
  ...
  } else if (o instanceof SVG.Color || o instanceof SVG.Number) {
    // @ts-ignore
    return o.at(pos);
  } else if (pos < 1) {
    return o.from;
...
(function (w) {
  ...
  for (var x = 0; x < vendors.length && !window.requestAnimationFrame; ++x) {
    ...
    cancelAnimationFrame = w[vendors[x] + 'CancelAnimationFrame'] || w[vendors[x] + 'CancelRequestAnimationFrame'];
  }
  ...
  // https://gitlab.xmind.cn/xmind/snowbrush/issues/639
  // delete this comment in future weeks if there is no bug about this issue
  // (function () {
  //   var map = {}; //存储所有的动画请求函数
