...
import type { LayoutEngine, LayoutResult, NodeLayout, ConnectionLayout, LayoutOptions } from './types'
...
export class MapLayoutEngine implements LayoutEngine {
  ...
  calculate(root: MindMapNode, options?: LayoutOptions, dirtyNodes?: Set<string>): LayoutResult {
    ...
    if (attachedChildren.length > 0) {
      const { rightChildren, leftChildren } = this.splitChildren(attachedChildren, opts)
      
      const rightBounds = this.calculateSide(root, rightChildren, 'right', nodes, connections, opts, dirtyNodes)
      const leftBounds = this.calculateSide(root, leftChildren, 'left', nodes, connections, opts, dirtyNodes)
  ...
  private splitChildren(children: MindMapNode[], options: LayoutOptions): { rightChildren: MindMapNode[]; leftChildren: MindMapNode[] } {
  ...
  private calcNumRight(children: MindMapNode[]): number {
    ...
    const totalWeight = children.reduce((sum, child) => sum + this.getWeight(child), 0)
  ...
  private centerRoot(
    ...
    if (rightBounds && leftBounds) {
      centerY = (rightBounds.y + rightBounds.height / 2 + leftBounds.y + leftBounds.height / 2) / 2
    } else if (rightBounds) {
    ...
    if (rightBounds) {
      rootLayout.x = rightBounds.x - rootLayout.width - options.horizontalSpacing
    } else if (leftBounds) {
      rootLayout.x = leftBounds.x + leftBounds.width + options.horizontalSpacing
    }
  }
...
export class TreeLayoutEngine implements LayoutEngine {
  ...
  private calculateChildren(
    ...
    for (const childLayout of childLayouts) {
      ...
      childLayout.y = parentLayout.y + parentLayout.height + options.verticalSpacing
...
export class FishboneLayoutEngine implements LayoutEngine {
  ...
  private calculateChildren(
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      childLayout.y = mainLineY + offset * Math.sin(angle * Math.PI / 180) - childLayout.height / 2
...
export class TimelineLayoutEngine implements LayoutEngine {
  ...
  constructor(orientation: 'horizontal' | 'vertical' = 'horizontal', options?: Partial<LayoutOptions>) {
    this.options = { ...DEFAULT_LAYOUT_OPTIONS, ...options }
  ...
  private calculateChildren(
    ...
    for (const child of children) {
      ...
      connections.set(connectionId, {
        ...
        path: this.calculateConnectionPath(parentLayout, childLayout),
        startPoint: {
          x: this.orientation === 'horizontal' ? parentLayout.x + parentLayout.width : parentLayout.x + parentLayout.width / 2,
          y: this.orientation === 'horizontal' ? parentLayout.y + parentLayout.height / 2 : parentLayout.y + parentLayout.height,
        },
        endPoint: {
          x: this.orientation === 'horizontal' ? childLayout.x : childLayout.x + childLayout.width / 2,
          y: this.orientation === 'horizontal' ? childLayout.y + childLayout.height / 2 : childLayout.y,
        },
        controlPoints: [],
      })
