...
function getDoc(target: Y.Doc | CollabDoc): Y.Doc {
  return target instanceof Y.Doc ? target : target.doc;
}

export function syncTopicToY(doc: Y.Doc | CollabDoc, topic: TopicData): void {
  const ydoc = getDoc(doc);
...
export function syncYToTopic(doc: Y.Doc | CollabDoc): TopicData | null {
  const ydoc = getDoc(doc);
  const nodes = ydoc.getMap<Y.Map<any>>("nodes");
  const rootId = ydoc.getText("rootId");
