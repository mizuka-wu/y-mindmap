...
export class MindMapNode {
  ...
  getAttributeTitleWithGlobalStyle(): {
    ...
    if (globalStyle) {
      const cleanedTitle = removeGlobalStyleFromAttributeTitle(
        this.attributeTitle!,
        globalStyle,
      );
  ...
  updateChild(
    childId: string,
    updater: (child: MindMapNode) => MindMapNode,
  ): MindMapNode {
    const newChildren: Record<string, TopicData[]> = {};
    for (const [type, children] of Object.entries(this.children)) {
  ...
  private serializeChildren(
    children: Record<string, MindMapNode[]>,
