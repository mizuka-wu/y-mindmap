...
export class Sheet {
  ...
  withDoc(doc: MindMapDocument): Sheet {
  ...
  updateRelationship(id: string, updater: (r: Relationship) => Relationship): Sheet {
    return new Sheet({
      ...this.toData(),
  ...
  updateBoundary(id: string, updater: (b: Boundary) => Boundary): Sheet {
    return new Sheet({
      ...this.toData(),
      boundaries: this.boundaries.map((b) =>
        b.id === id ? updater(b) : b,
      ),
    })
  }
  ...
  updateSummary(id: string, updater: (s: Summary) => Summary): Sheet {
    return new Sheet({
      ...this.toData(),
      summaries: this.summaries.map((s) =>
        s.id === id ? updater(s) : s,
      ),
    })
  }
  ...
  static fromData(data: SheetData): Sheet {
    return new Sheet({
      id: data.id,
      title: data.title,
      doc: MindMapDocument.fromJSON(data.rootTopic),
