import { 
  Plugin, PluginAPI, PluginMetadata, PluginEvent, PluginEventHandler,
  MenuItem, ToolbarButton, PanelOptions, DialogOptions, NotificationOptions
} from './types'
...
export class PluginManager {
  ...
  private commands: Map<string, (state: EditorState, dispatch?: (tr: Transaction) => void) => boolean> = new Map()
  ...
  private dialogHandler: ((options: DialogOptions) => Promise<void>) | null = null
  ...
  getCommand(name: string): ((state: EditorState, dispatch?: (tr: Transaction) => void) => boolean) | undefined {
    return this.commands.get(name)
  }

  getAllCommands(): Map<string, (state: EditorState, dispatch?: (tr: Transaction) => void) => boolean> {
  ...
  private createAPI(pluginId: string): PluginAPI {
    return {
      ...
      getSelectedNodes: () => {
        ...
        return ids.map(id => this.editorState!.doc.getNodeById(id)).filter((n): n is MindMapNode => n !== null)
      },
