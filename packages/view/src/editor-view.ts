...
import {
  ...
  DropPosition,
  DropTarget,
} from "./node-views/interactions/drag-node-views";
...
export class EditorView {
  ...
  private _onTitleUpdate: ((nodeId: string, title: string) => void) | null =
    null;
  ...
  constructor(config: EditorViewConfig) {
    ...
    const showZoomControls =
      config.enableZoomControls ?? config.showZoomControls ?? false;
    if (showZoomControls) {
  ...
  private _createZoomControls(config?: ZoomControlsConfig): void {
    ...
    this._zoomControls = new ZoomControls(
      this._zoomControlsContainer,
      {
        getZoom: () => this.getZoom(),
        zoomTo: (level) => this.zoomTo(level),
        zoomIn: () => this.zoomIn(),
  ...
  private _findNodeIdAtPoint(worldPoint: {
    x: number;
    y: number;
  }): string | null {
    ...
    for (const [id, view] of this.nodeViewFactory.getAllTopicViews()) {
      if (view.isVisible() && !view.isForcedInvisible()) {
        ...
        if (
          worldPoint.x >= absBounds.x &&
          worldPoint.x <= absBounds.x + absBounds.width &&
          worldPoint.y >= absBounds.y &&
          worldPoint.y <= absBounds.y + absBounds.height
        ) {
          return id;
        }
      }
      ...
      if (isMultiSelect) {
        ...
        tr.setSelection(
          newSelection.length > 0
            ? Selection.multiple(newSelection)
            : Selection.empty(),
        );
        this.dispatch(tr);
        return;
        ...
        if (isValid) {
          ...
          if (dropPos === DropPosition.INSIDE) {
            ...
            if (parent) {
              ...
              const insertIndex =
                dropPos === DropPosition.BEFORE ? targetIndex : targetIndex + 1;
  ...
  private _setOutsideBoxSelectNodesForcedInvisible(boxBounds: {
    x: number;
    y: number;
    width: number;
    height: number;
  ...
  private _showContextMenu(
    ...
    const items = [
      {
        label: "添加子节点",
        action: () => this._dispatchCommand("addSubTopic", nodeId),
      },
      {
        label: "添加兄弟节点",
        action: () => this._dispatchCommand("addSiblingTopic", nodeId),
      },
      {
        label: "删除",
        action: () => this._dispatchCommand("deleteNode", nodeId),
        danger: true,
      },
      {
        label: "折叠/展开",
        action: () => this._dispatchCommand("toggleFold", nodeId),
      },
  ...
  stopEditing(save: boolean): void {
    ...
    if (save) {
      ...
      const originalTitle =
        this.state?.doc.getNodeById(this._editingNodeId)?.title ?? "";
  ...
  private _setNonEditingNodesForcedInvisible(
  ...
  private _removeEditOverlay(): void {
    ...
    if (this._editKeyDownHandler) {
      this._editOverlay.removeEventListener(
        "keydown",
        this._editKeyDownHandler,
      );
