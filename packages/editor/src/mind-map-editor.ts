import { EditorState, MindMapDocument, MindMapNode, Selection, Transaction } from '@y-mindmap/state'
...
import { 
  InteractionManager, 
  createSelectHandler, createKeyboardHandler, createDragHandler, createZoomHandler,
  createMultiSelectHandler, createViewportDragHandler, createBoxSelectHandler,
  createTextEditHandler, InlineEditor, RichTextInlineEditor, InertialScroll, GestureRecognizer
...
import { createYDocBinding, syncTopicToY, syncYToTopic, YDocBinding, CollaboratorManager, CollaboratorState, CollaboratorUser, LockConflict } from '@y-mindmap/collab'
...
export class MindMapEditor {
  ...
  constructor(options: MindMapEditorOptions) {
    ...
    this.interactionManager = new InteractionManager(this.state, (tr) => this.dispatch(tr))
    ...
    this.pluginManager.setEditorContext(
      this.state,
      (tr) => this.dispatch(tr)
    )
  ...
  private initCollaboration(ydoc: Y.Doc, user: CollaboratorUser): void {
    ...
    this.collaborators.onStateChange(() => {
      if (this.collaborators) {
        const cursors = new Map<number, { clientId: number; user: { name: string; color: string }; position: { x: number; y: number }; nodeId: string | null }>()
        const selections = new Map<number, { clientId: number; user: { name: string; color: string }; nodeIds: string[] }>()
  ...
  executeCommand(name: string, args?: any): boolean {
    return this.commandRegistry.execute(name, this.state, (tr) => this.dispatch(tr))
  }
  ...
  isEditing(): boolean {
    return this.inlineEditor.isEditing() || this.richTextInlineEditor.isEditing()
  }
  ...
  startEditing(nodeId: string): void {
    ...
    if (bounds && node) {
      if (this.enableRichText && node.isRichTitle) {
        this.richTextInlineEditor.startEditing(nodeId, node.attributeTitle || node.title, bounds)
      } else {
  ...
  private handleRichTextUpdate(nodeId: string, attributeTitle: AttributeTitle): void {
    const node = this.state.doc.getNodeById(nodeId)
    if (!node) return
  ...
  private registerDefaultHandlers(): void {
    ...
    this.interactionManager.addHandler(createTextEditHandler({
  ...
  private bindDOMEvents(): void {
    ...
    this.container.addEventListener('pointerdown', (e) => {
      this.gestureRecognizer?.handlePointerDown(e.pointerId, e.clientX, e.clientY)
    ...
    this.container.addEventListener('pointermove', (e) => {
      this.gestureRecognizer?.handlePointerMove(e.pointerId, e.clientX, e.clientY)
    ...
    this.container.addEventListener('pointerup', (e) => {
      this.gestureRecognizer?.handlePointerUp(e.pointerId, e.clientX, e.clientY)
